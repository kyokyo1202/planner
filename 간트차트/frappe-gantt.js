var Gantt = (function () {
  ...
  class Bar {
    ...
    getColor(progress) {
      ...
      if (progress > 50 && progress <= 80) return "orange";
      if (progress > 80 && progress <= 90) return "yellow";
    }
    ...
    toggle_children(className) {
      ...
      arr.forEach((taskid) => {
        ...
        let ar = document.querySelectorAll(
          "[data-from='" + taskid + "'],[data-to='" + taskid + "']"
        );
        ar.forEach((i) => {
  ...
  class Popup {
    ...
    event_edit(parent, options, gantt) {
      ...
      $add.addEventListener("click", function () {
        ...
        parent.querySelector(".tools").innerHTML =
          '<svg id="valid" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 406.834 406.834" style="enable-background:new 0 0 406.834 406.834;" xml:space="preserve"><polygon points="385.621,62.507 146.225,301.901 21.213,176.891 0,198.104 146.225,344.327 406.834,83.72 "/></svg>';

        parent.querySelector(".title").innerHTML = "참여자 이름";

        for (let i = 0; i < size; i++) {
          //create drop downs
          let drop = "<select id=s" + i + ' value="">';
          let op = '<option value="">참여자 </option> ';
          op =
            op +
            p.map((p) => {
        ...
        const fn = (event) => {
          ...
          let arr = [];
          parent
            .querySelector("#divselect")
            .querySelectorAll("select")
            .forEach((j) => {
              if (j.value != "") arr.push(j.value);
            });
          selectedPeople = arr;
        ...
        parent.querySelector(".subtitle").innerHTML =
          "<div id='divselect'></div> <div id='btn'></div>";
        parent.querySelector("#divselect").innerHTML = d;

        parent.querySelector("#btn").innerHTML =
          '<svg id="more" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 52 52" style="enable-background:new 0 0 52 52;" xml:space="preserve"><g  stroke-width="6"><path d="M26,0C11.664,0,0,11.663,0,26s11.664,26,26,26s26-11.663,26-26S40.336,0,26,0z M26,50C12.767,50,2,39.233,2,26   S12.767,2,26,2s24,10.767,24,24S39.233,50,26,50z"/><path d="M38.5,25H27V14c0-0.553-0.448-1-1-1s-1,0.447-1,1v11H13.5c-0.552,0-1,0.447-1,1s0.448,1,1,1H25v12c0,0.553,0.448,1,1,1   s1-0.447,1-1V27h11.5c0.552,0,1-0.447,1-1S39.052,25,38.5,25z"/></g></svg>';
        ...
        for (let i = 0; i < size; i++) {
          ...
          console.log(
            parent.querySelector("#divselect").querySelectorAll("select")[i]
          );
        }
      ...
      $edit.addEventListener("click", function () {
        ...
        parent.querySelector(".title").innerHTML =
          '<input id="task_name" class="input1" type="text" value="' +
          options.task.name +
          '">';
        parent.querySelector(".subtitle").innerHTML =
          '<input id="task_start" class="input2" type="date" value="' +
          options.task.start +
          '">' +
          '<input id="task_end" class="input2" type="date" value="' +
          options.task.end +
          '">';
    ...
    make() {
      this.HtmlTools =
        '<div class="tools"></div> <div class="title"></div><div class="subtitle"></div><div class="pointer"></div>';
  ...
  class Gantt {
      ...
      this.tasks = tasks.map((task, i) => {
        ...
        if (arr.length > 0) {
          ...
          for (let a of arr) {
            ...
            if (
              Date.parse(date_utils.parse(aa.end)) >
              Date.parse(date_utils.parse(s.end))
            )
              s = aa;
          }
        }
    ...
    our_menu() {
      ...
      this.add_task.appendChild(document.createTextNode("프로젝트 추가"));
      this.add_sub.appendChild(
        document.createTextNode("프로젝트 세부내용 추가")
      );
      ...
      this.add_task_title.placeholder = "제목을 입력해주세요";
      ...
      this.add_task_completion.placeholder = "완성도 %";
    ...
    sub_task() {
      ...
      this.add_sub_task_title.placeholder = "제목을 입력해주세요";
      ...
      this.add_sub_task_completion.placeholder = "완성도 %";
    ...
    bind_events() {
      ...
      this.$container.addEventListener("contextmenu", (ev) => {
        ...
        if (!isOpen) this.clear_menu();

        if (this.tasks.length) this.add_sub.classList.remove("disabled");
        else this.add_sub.classList.add("disabled");
      ...
      this.add_task_formulaire.addEventListener("submit", (ev) => {
        ...
        if (!this.add_task_end.value) this.add_task_end.classList.add("wrong");

        if (!this.add_task_title.value)
          this.add_task_title.classList.add("wrong");

        if (
          this.add_task_title.value &&
    ...
    getColor(progress) {
      ...
      if (progress <= 50) colorFilled = "red";
      if (progress > 50 && progress <= 80) colorFilled = "orange";
      if (progress > 80 && progress <= 90) colorFilled = "yellow";
